...
namespace DF.Common {   
    ...
    public class IdWorkTool {
        ...
        static IdWorkTool snowflake;

        /// <summary>
        /// 
        /// </summary>
        /// <returns></returns>
        public static IdWorkTool Instance()
        {
            if (snowflake == null)
                snowflake = new IdWorkTool();
            return snowflake;
        }
        /// <summary>
        /// 
        ...
        public IdWorkTool(long machineId)
        {
        ...
        public IdWorkTool(long machineId, long datacenterId)
        {
        ...
        private void Snowflakes(long machineId, long datacenterId)
        {
            if (machineId >= 0)
            {
                ...
                IdWorkTool.machineId = machineId;
            }
            if (datacenterId >= 0)
            {
                ...
                IdWorkTool.datacenterId = datacenterId;
            }
        }
        ...
        public long GetId()
        {
            ...
            {
                long timestamp = GetTimestamp();
                if (IdWorkTool.lastTimestamp == timestamp)
                {
                    //同一微妙中生成ID  
                    ...
                    if (sequence == 0)
                    {
                        ...
                        timestamp = GetNextTimestamp(IdWorkTool.lastTimestamp);
                    }
                }
                else
                {
                ...
                IdWorkTool.lastTimestamp = timestamp; //把当前时间戳保存为最后生成ID的时间戳  
