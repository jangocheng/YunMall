...
namespace HS.Shop.Common.InvokeInterface
{
    ...
    /// 支付接口调用公共类
    /// </summary>
    public class PayInvokeInterface
    {
        /// <summary>
        /// 调用支付模块
        /// </summary>
        ...
        /// <param name="url">调用地址</param>
        /// <returns>输出参数</returns>
        public async static Task<TResult> InvokeAsync1<TPara, TResult>(TPara model, string url) where TResult : HS.PayModel.Base.MyBaseResp
        {
            //封装实体类
            BaseReqModel<TPara> Model = new BaseReqModel<TPara>()
            {
        ...
        /// <typeparam name="TResult">输出参数</typeparam>
        /// <param name="model">实体类</param>
        /// <param name="TicketIdUrl">ticketid请求地址</param>
        /// <param name="url">调用地址</param>
        /// <returns>输出参数</returns>
        public async static Task<TResult> InvokeAsync1<TPara, TResult>(TPara model, string TicketIdUrl, string url) where TResult : HS.PayModel.Base.MyBaseResp
        {
            string TicketId = HttpHelper1.Get(TicketIdUrl);
            //封装实体类
            BaseReqModel<TPara> Model = new BaseReqModel<TPara>()
            {
                AppName = ConstBaseData.AppName,
        ...
        /// <param name="url">调用地址</param>
        /// <returns>输出参数</returns>
        public async static Task<TResult> InvokeAsync2<TPara, TResult>(TPara model, string url) 
